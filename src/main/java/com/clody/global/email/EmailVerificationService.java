...
public class EmailVerificationService {
    ...
    // 비밀번호 변경용 인증번호 생성 및 저장
    public String generateAndSavePasswordResetCode(String email) {
        String verificationCode = generateVerificationCode();
        ...
        try {
            if (emailSendEnabled) {
                ...
                log.info("비밀번호 변경용 이메일 발송 완료 - email: {}", email);
            } else {
                // 개발환경에서는 콘솔에 인증번호 출력
                log.info("=== 비밀번호 변경용 인증번호 (콘솔 출력) ===");
                log.info("이메일: {}", email);
                log.info("인증번호: {}", verificationCode);
                log.info("만료시간: 5분");
                log.info("==========================================");
            ...
            log.error("비밀번호 변경용 이메일 발송 실패, 콘솔 로그로 대체 - email: {}, error: {}", email, e.getMessage());
            log.info("=== 비밀번호 변경용 인증번호 (발송 실패, 콘솔 출력) ===");
            log.info("이메일: {}", email);
            log.info("인증번호: {}", verificationCode);
            log.info("만료시간: 5분");
            log.info("======================================================");
    ...
    public boolean verifyPasswordResetCode(String email, String inputCode) {
        ...
        if (savedCode == null) {
            log.warn("비밀번호 변경용 인증번호가 만료되거나 존재하지 않습니다. email: {}", email);
            throw new MemberException(MemberErrorCode.EMAIL_VERIFICATION_CODE_EXPIRED);
        }
        
        if (!savedCode.equals(inputCode)) {
            log.warn("비밀번호 변경용 인증번호가 일치하지 않습니다. email: {}, input: {}, saved: {}", email, inputCode, savedCode);
            throw new MemberException(MemberErrorCode.EMAIL_VERIFICATION_CODE_INVALID);
        }
        ...
        // 비밀번호 변경 인증 완료 플래그 저장 (10분 유지)
        String verifiedKey = PASSWORD_RESET_PREFIX + "verified:" + email;
        redisTemplate.opsForValue().set(verifiedKey, "true", Duration.ofMinutes(10));
        
        log.info("비밀번호 변경용 인증이 완료되었습니다. email: {}", email);
        return true;
    }
    ...
    public void removePasswordResetVerifiedFlag(String email) {
        String verifiedKey = PASSWORD_RESET_PREFIX + "verified:" + email;
        redisTemplate.delete(verifiedKey);
    }
